...
public class Position {
	...
	return STRING_FORMAT.replace("X", String.valueOf(x))
     ...
     * @return The distance between two positions.
     */
    public static double calculateDistance(Position position1,
	    Position position2) {
	...
	return (Math
		.acos(calculateDotProduct(position1, position2)
			/ calculateMagnitude(position1)
			* calculateMagnitude(position2)))
		* Math.PI / 180;
    }
     ...
     *         positions.
     */
    public static double calculateDotProduct(Position position1,
	    Position position2) {
	return position1.getX() * position2.getX() + position1.getY()
		* position2.getY() + position1.getZ() * position2.getZ();
    }
     ...
     * 
     * @see https 
     *      ://en.wikipedia.org/wiki/Magnitude_(mathematics)#Euclidean_vector_space
     * 
     * @param position
     *            The position.
     * @return The magnitude of the coördinates of the specified position.
	...
	return Math.sqrt(Math.pow(position.getX(), 2)
		+ Math.pow(position.getY(), 2) + Math.pow(position.getZ(), 2));
    }
